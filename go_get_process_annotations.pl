...
my $go_process_count = 0;
my $uniprot_no_go_process_count = 0;

# open output files
my $go_filename = './output/go_process_annots.tsv';
my $no_go_filename = './output/go_process_annots_not_found.tsv';
open (GO_FILE,"> $go_filename") or die "Error opening output file\n";
open (NO_GO_FILE,"> $no_go_filename") or die "Error opening output file\n";
print "Printing Gene Ontology process annotations to output file $go_filename...\n";
...
while (my @row = $sql_stmt->fetchrow_array()) {
  ...
  # returning only the Biological Process annotations
  my $query = "http://www.ebi.ac.uk/QuickGO/GAnnotation?format=tsv&protein=$uniprot_acc&aspect=P&col=goID,goName,evidence";
...
print "Total GO Biological Process annotations found: $go_process_count, output file $go_filename\n";
print "UniProt entries without GO Biological Process annotation: $uniprot_no_go_process_count, output file $no_go_filename\n";
