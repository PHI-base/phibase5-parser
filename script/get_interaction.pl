...
my $my_string;
print "Please enter PHI-base accession in the form PHI:Ixxxx, where xxxx is the PHI-base accession number:\n";
chop($my_string = <STDIN>);


# open output file
my $db_data_filename = '../output/interaction_details.tsv';  
...
# if the PHI-base accession is in the new format, query the interaction table directly
# otherwise, if its in the old format, query based on the obsolete_accession
# else, the accession is not valid
my $sql_stmt;
if ($my_string =~ /^PHI:I/) {
  $sql_stmt = qq(SELECT id,phi_base_accession,curation_date FROM interaction WHERE phi_base_accession = '$my_string');
} elsif ($my_string =~ /^PHI:/) {
  $sql_stmt = qq(SELECT interaction.id,interaction.phi_base_accession,interaction.curation_date 
                   FROM interaction, obsolete 
                   WHERE obsolete.obsolete_accession = '$my_string'
                   AND obsolete.phi_base_accession = interaction.phi_base_accession
                );
} else {
  print STDERR "ERROR: PHI-base accession is invalid\n";
  die;
}
...
while (my @row = $sql_result->fetchrow_array()) {
  ...
  print DATABASE_DATA_FILE "$host_taxon_string\t$cotyledon_output_string\t$uniprot_go_annotation\t";
