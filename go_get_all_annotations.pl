...
my $uniprot_no_go_annot_count = 0;

# open output files
my $go_filename = './output/go_all_annotations.tsv';
my $no_go_filename = './output/go_annotations_not_found.tsv';
open (GO_FILE,"> $go_filename") or die "Error opening output file\n";
open (NO_GO_FILE,"> $no_go_filename") or die "Error opening output file\n";
print "Printing Gene Ontology annotations to output file $go_filename...\n";
...
while (my @row = $sql_stmt->fetchrow_array()) {
  ...
  #my $query = "http://www.ebi.ac.uk/QuickGO/GAnnotation?format=tsv&protein=$uniprot_acc&col=proteinSymbol,proteinTaxon,proteinTaxonName,goID,goName,evidence,aspect";
  my $query = "http://www.ebi.ac.uk/QuickGO/GAnnotation?format=tsv&protein=$uniprot_acc&col=goID,goName,evidence,aspect";
  ...
  if (@go_annotations) {
     ...
     print GO_FILE "GO ID\tGo Term\tGO Evidence Code\tGO Aspect\n";
...
print "Total GO annotations found: $go_annotation_count, output file $go_filename\n";
print "UniProt entries without GO annotation: $uniprot_no_go_annot_count, output file $no_go_filename\n";
