...
my $go_component_count = 0;
my $uniprot_no_go_component_count = 0;

# open output files
my $go_filename = './output/go_component_annots.tsv';
my $no_go_filename = './output/go_component_annots_not_found.tsv';
open (GO_FILE,"> $go_filename") or die "Error opening output file\n";
open (NO_GO_FILE,"> $no_go_filename") or die "Error opening output file\n";
print "Printing Gene Ontology component annotations to output file $go_filename...\n";
...
while (my @row = $sql_stmt->fetchrow_array()) {
  ...
  # returning only the Cellular Component annotations
  my $query = "http://www.ebi.ac.uk/QuickGO/GAnnotation?format=tsv&protein=$uniprot_acc&aspect=C&col=goID,goName,evidence";
...
print "Total GO Cellular Component annotations found: $go_component_count, output file $go_filename\n";
print "UniProt entries without GO Cellular Component annotation: $uniprot_no_go_component_count, output file $no_go_filename\n";
